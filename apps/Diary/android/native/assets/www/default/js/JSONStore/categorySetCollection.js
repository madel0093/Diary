...
/* JavaScript content from js/JSONStore/categorySetCollection.js in folder common */
...
WL.namespace('WL.CategoryCollection');
WL.CategoryCollection = (function(WL, jQuery, lodash) {
    'use strict';
    ...
    var COLLECTION_NAME = 'categorySet';
    ...
    var _showTable = function(arr) {
        ...
        // Get reference to the status field
            ...
            table = ['<table id="user_table" >', '<tr>',
                ...
                '<td><b>name</b></td>',
                '<td><b>idGeneralCategory</b></td>',
                '<td><b>json</b></td>', '</tr>',
                '<% _.each(people, function(person) { %>', '<tr>',
                '<td> <%= person._id %> </td>',
                '<td> <%= person.json.idUser %> </td>',
                '<td> <%= person.json.idCategory %> </td>',
                '<td> <%= person.json.name %> </td>',
                '<td> <%= person.json.idGeneralCategory %> </td>',
        ...
        WL.JSONStore.init({
            ...
            categorySet: {
                searchFields: {
                    ...
                    "name": "string",
                    "idCategory": "integer",
                    "idGeneralCategory": "string"
                },
                adapter: {
                    name: 'DiaryMySQL',
                    add: 'addCategoryInfo',
                    remove: 'deleteCategoryInfo',
                    replace: 'updateCategoryInfo',
                    load: {
                        procedure: 'getCategoryInfos',
    ...
    var _findAll = function(resFun) {
        ...
        try {
            WL.JSONStore.get(COLLECTION_NAME).findAll(options)
        ...
        try {
            ...
            _findQuery({
                ...
                if (cat.result == "YES") {
                    ...
                    doc.json.name = data.name || cat.name;
                    doc.json.idUser = cat.list[0].json.idUser;
                    doc.json.idCategory = cat.list[0].json.idCategory;
                    doc.json.idGeneralCategory = cat.list[0].json.idGeneralCategory;
    ...
    var _remove = function(data, resFun) {
